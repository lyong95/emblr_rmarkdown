...
## What is markdown?
a language aimed at making writing in a text editor easier and readable with simple
...
![rmd workfllow](/g/typas/Personal_Folders/Luoyan/emblR/Rmarkdown/images/workflow.png)
...
Try out other options using the code chunk above 

![chunk options](/g/typas/Personal_Folders/Luoyan/emblR/Rmarkdown/images/chunk_options_table.png)
...
-introduce 'age' variable in params and params$age is now recognised as a variable
in the code chunks
...
```{r}
rmarkdown::render(input = "example_report.Rmd", output_file = "colon_example_report.Rmd", params = list(data ="/g/typas/Personal_Folders/Luoyan/emblR/Rmarkdown/data/colon.csv" ))
...
reports <- tibble(
  ...
  params = purrr::map(paste0("data/", data_type, ".csv"),  ~ list(data = .))
)
...
You can add presenter's notes by enclosing your notes with the following
```{r, eval = FALSE}
<div class="notes">
...
- highlight brief and key summaries of a report
- Embed a wide variety of components including HTML widgets, R graphics, tabular data

- level 1 header - new page
- level 2 header - new column
- level 3 header - new row

You can also add interactive components in 
```{r}
p = ggplot(diamonds[1:50, ], aes(x = price, y = x))+
  geom_point()
